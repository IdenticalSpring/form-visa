...
export const Step7Form = ({ data }: { data: UserInfo }) => {
  ...
  const {
    ...
    defaultValues: {
      ...
      is_lived_in_visa_coutry_date: data.is_lived_in_visa_coutry_date
        ? new Date(data.is_lived_in_visa_coutry_date)
        : new Date(),
      is_lived_in_visa_coutry_days_stay:
        data.is_lived_in_visa_coutry_days_stay ?? 0,
    ...
    if (isInitialLoad) {
      ...
      if (storedData) {
        ...
        if (parsedData.expected_start_date) {
          parsedData.expected_start_date = new Date(
            parsedData.expected_start_date
          );
        }
        if (parsedData.is_lived_in_visa_coutry_date) {
          parsedData.is_lived_in_visa_coutry_date = new Date(
            parsedData.is_lived_in_visa_coutry_date
          );
        }
